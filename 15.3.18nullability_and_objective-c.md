...
在之前版本的 Xcode 中，有些苹果官方的框架经过了专门的审核，来让 API 正确反映 Swift 的可选类型。为了支持在你自己的代码里也能实现这种功能，Xcode 6.3 推出了一项新的 Objective-C 语言特性：_nullability annotations_ 。
...
为了循序渐进地采用这些新注解，你可以给 Objective-C 头文件中的一个区块加上“经过非空性审核”的标记。在这个区块内部，每个普通的指针类型都会默认为非空。这能让上文的例子大大简化：
